...
def index(request):
	...
	PM_list = Project.objects.filter(~Q(status='C')).order_by('PM__username').values('PM__username').distinct()
	AM_list = Project.objects.filter(~Q(status='C')).order_by('AM').values_list('AM', flat=True).distinct()
